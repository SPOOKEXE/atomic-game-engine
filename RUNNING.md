...
Both files are regenerated by `just bindings` and are checked for drift by `just
bindings-check`, which `just check` runs. **A change to either is a change to
what every script can name**, so the diff is the review.
