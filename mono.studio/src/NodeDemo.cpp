// The Demo Nodes panel: a canvas over `Mono::nodegraph`, and the pictures it
// draws.
//
...
// it proves is that the library is enough for both: a registry, a model with a
// cycle guard, a layout, a canvas, and an evaluator that can run something slow
// without stopping the frame.
//
// **The canvas is not here.** It is `mono.studio/nodegraph`, which is where this
// design lives after D00113, and this file is only the panel around it: a
// texture for a picture, a theme, a file dialog and an undo stack, which is what
// a host has and a library cannot. The node set is not here either - it is
// `src/DemoNodes.cpp`, beside this file rather than inside the library, because
// it is content.
//
// ## The panel
//
