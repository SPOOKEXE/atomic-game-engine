...
namespace studio {
	...
	void Editor::DrawColliderOutlines(size_t viewport, const PanelProjection &panel) {
		...
		Universe->Enter(world, [&](engine::ecs::Store &store) {
			...
			store.Each<const engine::scene::Transform, const engine::scene::Collider>(
				[&](engine::ecs::Entity part,
					...
					case engine::scene::ShapeKind::Box: {
						...
						for (int face = 0; face < 6; face++) {
							const ImU32 colour = ColourOf(tint, static_cast<uint32_t>(face));
							for (int edge = 0; edge < 4; edge++) {
								segment(
									corners[BOX_FACES[face][edge]],
									corners[BOX_FACES[face][(edge + 1) % 4]],
									colour
								);
							}
						}
					...
					case engine::scene::ShapeKind::Hull: {
						...
						if (hull == nullptr) {
							// **The bound, because that is what it collides
							// as.** A name that resolves to nothing collides as
							// the part's extent - `scene::Collider::Geometry`
							// states it - and drawing nothing here would hide
							// exactly the case an author most needs to see.
							Vector3 corners[8];
							BoxCorners(frame, collider.Extent, corners);
						...
						for (size_t face = 0; face < hull->Faces.size(); face++) {
							const engine::collision::HullFace &loop = hull->Faces[face];
							const ImU32 colour = ColourOf(tint, static_cast<uint32_t>(face));

							for (uint32_t at = 0; at < loop.IndexCount; at++) {
								const Vector3 &from = hull->Points[hull->Loops[loop.FirstIndex + at]];
								const Vector3 &to =
									hull->Points[hull->Loops[loop.FirstIndex + (at + 1) % loop.IndexCount]];
								segment(
					...
					case engine::scene::ShapeKind::Mesh: {
						const engine::collision::TriangleMesh *mesh =
						...
						const size_t triangles = std::min(mesh->TriangleCount(), OUTLINE_TRIANGLES);
						for (size_t triangle = 0; triangle < triangles; triangle++) {
							const engine::collision::Triangle corners = mesh->TriangleAt(triangle);
							const ImU32 colour = ColourOf(tint, static_cast<uint32_t>(triangle));

							const Vector3 a = frame.Position + frame.VectorToWorldSpace(corners.A);
							const Vector3 b = frame.Position + frame.VectorToWorldSpace(corners.B);
							const Vector3 c = frame.Position + frame.VectorToWorldSpace(corners.C);
							segment(a, b, colour);
							segment(b, c, colour);
							segment(c, a, colour);
						}
