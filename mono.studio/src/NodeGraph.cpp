...
namespace studio::nodes {
	...
	std::vector<NodeId> Graph::Absorb(const Graph &other, float dx, float dy, NodeId owner) {
		...
		for (const nodes::Group &frame : other.Groups()) {
	...
	namespace {
		...
		// what somebody expects an exported picture to be, and `mono.engine/bake`
		// only *reads* one — its decoder is a publishing-pipeline concern behind
		// `Engine::bake`, which this program's editor half does not link and
		// should not start linking for a file dialog.
		//
