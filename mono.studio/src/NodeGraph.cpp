...
namespace studio::nodes {
	...
	RunReport Evaluator::Run(const Graph &graph) {
		...
		for (const NodeId id : order) {
			...
			// The inputs, gathered from what the nodes upstream produced this
			// run. An unconnected input is simply absent — `Inputs::In` answers
			// the caller's fallback, which is the ordinary case and not an
			// error.
			Inputs inputs;
