...
namespace nodegraph {
	...
	struct Inputs {
		...
		// exists to avoid.
		//
		// Empty for a sync node. Calling it is still safe and does nothing, so one
		...
		//         knob, which is what a document naming a knob this build's node
		//         type no longer declares produces.
	...
	struct NodeType {
		...
		std::function<nodegraph::Outputs(const nodegraph::Inputs &)> Evaluate;
		...
		// on a widget the evaluation did not read: a thumbnail that disagreed with
		// the result would be worse than none.
		//
		...
		// places itself (the compressed node is the one here) does not have to be
		// taught to two panels separately.
		bool Hidden = false;
	};
	...
	class NodeTypes {
	  public:
		...
		// @return The type, or null, which is what a document naming a type this
		//         build does not have produces.
