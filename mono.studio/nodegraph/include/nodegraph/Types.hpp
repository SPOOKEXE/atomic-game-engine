...
namespace nodegraph {
	...
	struct PreviewImage {
		...
		// `Side * Side * 4` bytes, red first, top row first. That is the layout
		// every texture upload this has met wants, so a host uploads it as it
		// stands.
	...
	struct DataType {
		...
		// belongs. A node type may still override it (see `NodeType::Preview`) for
		// the case where two nodes on one wire type want different pictures.
	...
	class DataTypes {
	  public:
		...
		// @return The type, or null when nothing registered it, which is what a
	...
	struct Value {
		...
		// The number, for `Slider` and `Number`. Also the chosen index for
		...
		nodegraph::Colour Tint;

		// Whether both carry the same value.
		//
		// **Compares only the field `Kind` names**, so two values that differ in
		// a field neither is using are equal, which is what makes this usable as
		// the "did this knob move" test the undo log needs.
	...
	// popup, a section heading and an inspector row, all of them chrome drawn
	// *around* a graph rather than any part of one. Dear ImGui's own style, which all of
