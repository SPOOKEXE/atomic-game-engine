...
namespace nodegraph {
	...
	bool PictureOf(
		...
		// everything that does not gets the wire's, which is one function serving
		// every node that carries the same thing.
	...
	bool RenderSurface(
		...
		// `sqrt(2)` wider than a yaw of zero. Scaling to the flat-on width would
		// clip the corners off exactly half the time somebody orbits.
	...
	std::vector<uint8_t> EncodePng(const PreviewImage &image) {
		...
		// The raw scanlines, each with its filter byte. Filter 0 (none), because
		// a stored block gains nothing from a filter that only helps an entropy
		// coder.
