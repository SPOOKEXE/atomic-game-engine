...
namespace nodegraph {
	...
	namespace {
		...
		// registered, which is a fact rather than a failure: a saved graph can name
		// one this process has not heard of.
		ImVec4 InspectorTintOf(const std::string &type) {
		...
		void Dot(const std::string &type) {
			...
			ImGui::GetWindowDrawList()->AddCircleFilled(
				...
				size * 0.24f,
				ImGui::GetColorU32(InspectorTintOf(type))
			);
		...
		// overrode `NodeType::Preview` has to be the one asked about it, or a node
		// would show its neighbour's field through its own colour ramp.
		struct Sampled {
			std::string Port;
			std::string Type;
		...
		// over one graph do not fight over one pinned port or one camera, and so
		// it resets with the window rather than living for the process.
		struct Pin {
			int Index = 0;
			NodeId Node = NO_NODE;
			bool Three = false;
		...
		void Strip(
			...
			const auto across = static_cast<int>(std::max(1.0f, std::min(3.0f, room / Scaled(74.0f))));
			...
			for (size_t at = 0; at < drawable.size(); at++) {
				...
				ImGui::PushStyleColor(ImGuiCol_Border, InspectorTintOf(sample.Type));
		...
		void DrawField(const Inspection &what) {
			...
			ImGui::PushStyleColor(ImGuiCol_Text, InspectorTintOf(shown.Type));
		...
		void DrawRun(const Inspection &what) {
			...
			ImGui::PushStyleColor(
				ImGuiCol_Text,
				status.State == NodeState::Failed ? HostChrome().Warning
				: status.State == NodeState::Idle ? HostChrome().Muted
												  : HostChrome().Accent
			);
			...
			ImGui::ProgressBar(std::clamp(status.Progress, 0.0f, 1.0f), ImVec2(-1.0f, Scaled(6.0f)), "");
		...
		bool RegisteredInspectors = false;
	}
	...
	void RegisterInspectors() {
		...
		if (RegisteredInspectors) {
			return;
		}
		RegisteredInspectors = true;
