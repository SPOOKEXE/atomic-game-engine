...
namespace nodegraph {
	...
	RunReport Evaluator::Run(const Graph &graph) {
		...
		// Whatever finished since the last call, before anything is scheduled, so
		// a node whose input landed a moment ago starts on this run rather than
		// on the next one.
		Collect(report);
		...
		for (const NodeId id : order) {
			...
			if (type == nullptr || !type->Evaluate) {
				// **The skip says why, on the node.** A graph naming a type this
				// build does not have is the documented failure, and until v0.19
				// it was a number in `RunReport` with nothing to say which node
				// or which type - so a pipeline that loaded with a hole in it
				// looked identical to one whose node simply produces nothing.
				//
				// This library links nothing of ours and writes no log; the note
				// is how it reports, and the canvas is what shows it. See
				// `AGENTS.md`.
				NodeStatus &status = States[id];
				status.State = NodeState::Failed;
				status.Progress = 0.0f;
				status.Cached = false;
				status.Note = type == nullptr ? "no node type '" + node->Type + "' in this build"
											  : "node type '" + node->Type + "' has no evaluation";
				report.Skipped++;
				continue;
			}
