...
namespace nodegraph {
	...
	namespace {
		...
		// that is not the type somebody is pointing at: dimming the rest reads as
		// one answer, where colouring the match reads as forty.
		...
		// leaves an output rightwards and arrives at an input from the left, which
		// is what makes a graph readable when two nodes are stacked.
	...
	namespace {
		...
		// a wire wholly inside one is not drawn at this depth at all, which is the
		// whole visual effect of compression, and it is this one function
	...
	void Canvas::Copy(const Graph &graph) {
		...
		// port pointed into the original, which reads as a duplicate and behaves
		// as a second view of the first.
		...
		// as a wire from a node that is not there, and the alternative (keeping the
		// outside end) pastes a copy already wired into the original, which is
		// never what duplicating means.
	...
	void Canvas::Paste(Graph &graph) {
		...
		// part that is easy to get wrong (remapping `Owner`, the proxies and the
		// promotion keys) is the part that must exist once.
	...
	void Canvas::Duplicate(Graph &graph) {
		...
		// with both ends inside" rule to be got wrong. What was on the clipboard
		// is put back, because duplicating is not copying.
	...
	void Canvas::DrawGroups(const Graph &graph) const {
		...
		for (const nodegraph::Group &frame : graph.Groups()) {
			float left = 0.0f;
			float top = 0.0f;
			float right = 0.0f;
			float bottom = 0.0f;
	...
	void Canvas::DrawLinks(const Graph &graph, size_t hovered) const {
		...
		for (size_t index = 0; index < graph.Links().size(); index++) {
			...
			// Coloured by what it carries, which is the same rule the ports use: a
			// link's colour is a fact about the data and never about which node
			// it came from.
	...
	void Canvas::DrawNode(const Graph &graph, const Node &node, const NodeLayout &layout) const {
		...
		for (const PlacedPort &port : layout.Ports) {
			...
			// units apart, so the names would overlap each other and the body. A
			// collapsed node is one nobody is currently reading.
	...
	void Canvas::Palette(Graph &graph) {
		...
		if (PaletteNeeds != Needs::Anything && !PaletteType.empty()) {
			...
			ImGui::PushStyleColor(ImGuiCol_Text, HostChrome().Muted);
			ImGui::Text(
		...
		if (ImGui::BeginChild("##list", ImVec2(Scaled(200.0f), Scaled(280.0f)))) {
