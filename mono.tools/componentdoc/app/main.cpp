...
namespace {
	...
	std::string Render(const std::vector<Row> &rows) {
		...
		out << "**Two kinds of component are deliberately absent.** Ones a *program*\n";
		out << "registers rather than the engine: `mono.server` has two and `mono.client` one,\n";
		out << "and linking a program library into a build tool to document three rows is the\n";
		out << "worse trade. And ones registered lazily under the compiler's spelling of the\n";
		out << "type, by `Components::Of<T>()` at first use rather than by an explicit name -\n";
		out << "those appear here only if something this tool links happens to touch them.\n";
		out << "`WorldTime` below is one that does. A component in that state is a defect\n";
		out << "rather than a category: decision 21 says a name that crosses a save file is a\n";
		out << "string, and the compiler's spelling is not stable across compilers.\n\n";
		out << "| Column | Meaning |\n|---|---|\n";
