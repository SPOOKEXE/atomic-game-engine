...
- [x] deferred item catch up - **three shipped, two register defects fixed, and every remaining entry checked against the code rather than against its own summary.** `UIStroke.LineJoinMode` and `.StrokeSizingMode`: all three joins are one ring with its corner points moved, so a join costs no vertices and no second path - the trigger asked for "an outline builder that takes a join rule" and `PushRoundedOutline` already had the seam. `UIPageLayout.Animated`/`TweenTime`/`EasingStyle`/`EasingDirection` and `ScrollingFrame.ElasticBehavior`'s spring: both are pure functions of elapsed time through `CompileRequest::Seconds`, which is `render::FlipbookFrameAt`'s shape, so `gui` still reads no clock - and elastic needed a canvas drag, which nothing had, because the only thing that ever moved a canvas was the wheel. `SelectionBox.LineThickness`/`SurfaceColor3`/`SurfaceTransparency`: deferred behind "a triangle path for adornments" when what was missing was any path at all - `AdornmentGeometry` had no caller in the repository and no pass drew a world-space line, so a `SelectionBox` drew nothing whatever its properties said. `Editor::DrawAdornments` is its first consumer, through the overlay `DrawColliderOutlines` already uses. **The register itself had two defects**: `D00120` was a heading in both the live and the retired file - the second time, `D00109` records the first - so the live entry is renumbered `D00129` and the four `mono.engine/scene` comments that cited it now name the retired file; and that collision had let a bullet claim "there is no `Tool` class" two versions after one shipped. Nothing was retired: every open entry still has an open half, and `D00019`'s trigger was checked rather than assumed - upstream luau-lsp has moved 30 commits and has not moved its nested Luau, so our pin is still the editor's. `BorderOffset` was deleted from `D00129` because Roblox has no such property and a deferred property that does not exist is a trigger that can never fire
