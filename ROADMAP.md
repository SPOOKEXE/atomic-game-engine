...
- [_] export rigs, skeletons, keypoints, skinning data and animation tracks. The client MCP now returns bounded `data-rig/v1` skeleton and bone frames with stable IDs, exact tick rationals, deterministic ordering and explicit unavailable skinning, keypoint and clip fields. Authored keypoint storage, mesh skin weights, decoded animation channels, script bindings and demo coverage remain open.
