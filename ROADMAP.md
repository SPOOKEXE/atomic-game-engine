...
- [_] define MCP idempotency, expected versions, structured status, cancellation, capability limits, permissions and audit records. Capture submission, nested `capture_bundle`, cancellation and release carry bounded unique operation IDs and exact ticket identity checks. Nested bundle replay returns the stored result for identical options and rejects reuse with changed options; the full cross-tool policy remains open.
