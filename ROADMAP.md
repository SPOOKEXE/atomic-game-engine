...
- [_] can we do something to help async-compute more complex computations like noise terrain generation? it freezes main thread. maybe have a async job reader that steps every luau heartbeat and checks if the job is complete instead of completely freezing the app? this way we can multi-process / multi-thread math.noise and we just asynchronously / operationally wait for the results using job handlers + threads.
