...
- [_] complete an autonomous MCP capture workflow around the starter demo. The Python workflow prefers the full advertised lifecycle, creates a caller-named isolated world, binds every later call to that world, submits the pinned rerunnable package, optionally warms presentation before re-pausing, snapshots, requires the three stable object IDs, records raycast, AABB and OBB evidence, polls one nine-plane capture with a monotonic deadline, fetches BLAKE3-verified byte ranges, releases or cancels the ticket and retires its world on success or failure. The checked live example writes `scene.png`, depth, normal, albedo, packed material, emissive, object-ID, semantic-ID and part-ID PNG views plus `scene.json` with raw plane metadata, label coverage, lifecycle, camera, object, geometry-query and checksum data. It uses an existing local world only when the complete lifecycle is absent and never retires that fallback. Independent agent artifact parity remains open.
...
- [_] maintain acceptance fixtures for replay roundtrip, no-time-advance, image-label alignment, retry isolation, invalid data and Python or agent parity. Headless fixtures cover rerunnable package replacement, exact object, semantic and part ID pixels, stable sidecars, malformed and partial label replies, typed Python parsing and cleanup after malformed terminal captures. The live Python example verifies checksums and writes all nine aligned channel views with per-label pixel bounds and counts. A live MCP-agent artifact-parity run remains open.
