...
- [_] add typed, read-only observation hooks at proven render-graph boundaries. `DataFactoryHookBind` now registers and discovers twelve stable data-capture hooks, pins exact pipeline, revision, view, snapshot and node identity, observes the static output seam, and returns bounded asynchronous readbacks through Luau and MCP capability discovery. The release CPU lifecycle benchmark fell from 41.62 to 20.48 microseconds per 64 lifecycles after dispatch stopped copying the full graph snapshot; validated capture planes now take ownership of readback vectors instead of copying them. GPU and readback release profiling plus a second real consumer remain open. A hook is an observer node or declared node output in the existing graph, not a second callback graph beside it.
