...
- [_] add the remaining MCP tools and examples for multicamera, multiworld, optical flow, lighting contribution and interop, plus independent MCP-agent artifact coverage for the existing segmentation tools. The Python factory can now coordinate bounded captures for up to eight unique named cameras through the existing MCP tools, with deterministic ordering, exact per-ticket identity, complete frozen descriptors, verified owned bytes and cancellation-safe cleanup. A native atomic multicamera MCP tool remains open.
