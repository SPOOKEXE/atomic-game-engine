...
- [x] finish render cleanup terminal accounting. A staged parent import failure applies one image drop and three layer releases; an accepted changed-eye capture cancels a pending old-body job and retires its old tree exactly once. Fixed repeated fixed-rate particle timeline allocations exposed by the gate. The full dev Vulkan render run passed 1,679,108 assertions in 159 cases.
...
- [x] Can you make a just benchmark job that builds a docs/ENGINE_STRESS_TEST.md markdown file with the stress
  ...
  benchmark variants.
- [x] finish MCP cleanup acceptance with broad CI on the final tree. Three paired release control and Vulkan capture runs under normal desktop load are recorded with contention and timing spread. A headless Vulkan test passes 94 assertions across three capture activation, close, and drain cycles. On 2026-09-25 every `just preset=ci check` step passed on the committed tree except one order-dependent Studio pipeline test, since fixed in `6a468a42`. On 2026-09-26 the full rerun passed every step on the final tree: 7692 of 7692 cases across 683 suites, plus determinism, replay and orphans. It first needed `just format` over 21 files and purpose lines for `scene.ObservationTrace` and `script.PortalObservationLog`.
...
- [x] pass the portal acceptance matrix in `docs/to-delete/v025-SEAMLESS-PORTALS.md`. On 2026-09-26 the product impairment grid passed all 24 cells, with the strict 30/60 walk, matched 30 fps, 144/240 FPS, the stall schedule, the fault round trip and the protocol grid. Another room is now drawn from a local replica first: `ResolveDestinationWorld` prefers a ready copy in this process over a remote endpoint, the retained copy of the room just left is kept while the eye's world links to it, and a remote image is only the fallback before any copy has state. An eye that crosses before any state for the room has arrived is held at the doorway. The walk counts those frames as `first_contact_frames`, the only frames allowed to be empty. While a sealed source body is frozen, the server holds the input it reports consumed at H (`FrozenPortalPlayerInput`, `Authority::HoldConsumedInput`), so the client keeps predicting the forwarded input the destination catches up to instead of being pulled back and then snapping. A resting player's motion sample is captured with zero velocity, so pose acknowledgements continue while a player stands still after a transfer.
- [x] pass the strict process-hosted portal product capture at 30 and 60 Hz reliably. `[portal-product-image-handoff]` passed 3 of 3 on 2026-09-26 after the fixes in the item below.
- [x] trace and fix the intermittent portal product walk failures. Fixed on 2026-09-25 and 2026-09-26: forwarded input drained one row per tick; forwarding closed when native input was queued rather than applied; a grounded suffix landed on the pane plane; the walk steered around a point past the return pane; the destination replayed the frozen-source backlog in real time (the sealed baseline now states its applied input tick, older rows are skipped as `Included`, and the frozen backlog is moved through at once as `CaughtUp`); the forwarded input ring held 64 rows, less than a 300 ms seal, and now holds 256; QUIC flushed datagrams ahead of reliable streams and used Cubic (now reliable channels first, bulk snapshot chunks last, BBR); a camera standing inside a thick pane's part culled the pane's far face (portal and mirror surfaces now draw two-sided through no-cull pipeline twins); `graph::VisiblePane` tested a box, so a pane beside and behind a turned camera demanded a nested capture a round trip away (it now clips the rectangle); eye images always came from the remote endpoint even with an admitted local replica (they now use the replica, as pane images do); the retained source replica was dropped as soon as its seam left view, so a camera leading its body back through the pane drew a remote image that cannot show nested portals (it is now kept while the eye world links to it); and readiness blocked adoption once a crossed body walked past the enter range under latency.
...
- [_] add typed render hooks at proven boundaries. `DataFactoryHookBind` registers and discovers twelve stable read-only data-capture hooks and the synchronous `view.camera` mutation hook. Data capture remains pinned to exact pipeline, revision, view, snapshot and node identity and returns bounded asynchronous readbacks. `view.camera` accepts owned one-shot camera-frame, camera, and projection patches only for an exact installed pipeline revision, world, snapshot, and view slot; conflicts, stale revisions, malformed values, cancellation, and generation reuse are explicit. The renderer copies only the matching view before capture preparation and never calls external code or exposes a mutable view. The release CPU lifecycle benchmark fell from 41.62 to 20.48 microseconds per 64 lifecycles after dispatch stopped copying the full graph snapshot; validated capture planes now take ownership of readback vectors instead of copying them. GPU and readback release profiling plus a second real consumer remain open. A hook is an observer node, declared node output, or this bounded typed pre-view patch, not a second callback graph beside it.
  ...
  8. Design separate typed observation hooks for physics and replication only after the render hook has two real consumers. Reuse the naming, bounded queue and polling rules, while keeping each subsystem's own tick, thread and lifetime contract. The item below designs them now, because portal debugging already needs them.

- [x] add typed physics and replication observation hooks with an optional trace id. Physics and replication observations landed in `1dd7743d`. Portal observations add `portal.crossing`, `portal.arrival`, `portal.input` and `portal.handoff` records in a bounded ring per world (`script/PortalObservation.hpp`), with the trace id carried by `scene::ObservationTrace` through a transfer or taken from the world default. `server --observe-dir --observe-trace` writes every local world's portal and physics records to one JSONL file per process, and the portal walk reads that trail instead of log lines.
  1. Name each hook with an internal enum and a stable string for discovery, manifests and MCP, as the render hooks do. Physics: `physics.step` (per-stage cost and counts), `physics.body` (pose, velocity, grounded for subscribed bodies), `physics.contact` (begin and end), `physics.placement` (sweep from, displacement, hit, fraction, normal, slides). Portal: `portal.crossing` (seam, prior and current side offsets, direct or reconstructed, begun or refused with its reason), `portal.arrival` (placed pose and side). Replication: `replication.input` (route native, scheduled or forwarded, input tick, world tick, direction, lag), `replication.handoff` (offer, prepare, commit, done, move acknowledgement, input closed), `replication.publish` (rows, bytes, dropped per client), and the client's `replication.adoption`.
  2. Each record is its own fixed-size struct stamped with world, tick and a per-world sequence. No generic value bag and no inheritance tree.
  3. The optional trace id is a caller-chosen 64-bit value with a discovery label. It is set on a subscription, on a body, or on an input stream. Portal transfer messages, forwarded moves and handoff sessions carry it across worlds and processes, so one crossing reads as a single ordered trail. Records without one carry zero.
  4. Records go into a bounded ring per world and subscription with a drop counter. The host polls rings at the world barrier; nothing calls external code on a world or physics thread. A disabled hook costs one predictable branch, measured by a benchmark row.
  5. Hooks observe only. Changes still go through normal scene, physics or replication state.
  6. First consumers: the portal walk tests stop parsing `portal-input` log lines and read `replication.input`, `replication.handoff` and `portal.crossing` by trace id. `just engine-stress-bench` reads `physics.step` for per-stage costs. Then profile record bytes, drops and cost before adding more hook points.
