...
- [x] pass the strict process-hosted portal product capture at 30 and 60 Hz. The final run, `/tmp/atomic-portal-product-final-1-NkxJ4k`, passed 21,853 assertions in 1 case, including rendered continuity, both adoptions, body checks, and capture checks.

- [_] finish MCP cleanup acceptance with broad CI on the final tree. Three paired release control and Vulkan capture runs under normal desktop load are recorded with contention and timing spread; they show bounded resource counts and no causal timing claim. A rebuilt headless Vulkan test passes 94 assertions across three real capture activation, close, and drain cycles, including ready-ticket access after world retirement. Server and CDN headless artifact checks pass.
- [_] pass the remaining portal acceptance matrix for 30/60/144/240 frame rates, network impairment, resolution, profiling, and GPU in `docs/v025-SEAMLESS-PORTALS.md`. The requested 240 FPS run achieved 60.58 FPS.
