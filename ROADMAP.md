...
- [_] `Each` at 500k is at the memory wall, not a software one — 500k rows over two components is 12 MB touched per pass at ~59 GB/s, and `EachBatch` measures identically, so the per-row dispatch is not the cost. Improving it means changing the layout, which is v0.4's `scene` work rather than a tuning pass
