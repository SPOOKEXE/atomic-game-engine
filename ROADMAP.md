...
- [_] optimise particles and physics
- [_] GPU-resident particles: the pool lives on the device and the step is a compute pass. The frame's two remaining particle costs are both the CPU touching every particle - `particles.pack` copies the whole live pool to a staging buffer and `particles.age` simulates it, 0.83 ms and 1.0 ms in `release` at 512,000 particles against 0.02 ms to record the draw. Fill is not a factor, so screen-space splatting would solve a bottleneck this engine does not have. `mono.engine/effects` is a `shared` tier with no device, so the step belongs in `render`; effects keeps the emitters, blocks, free lists and spawn plans
- [_] particle step in compute with no compaction: `BlockSizeFor` is `Rate * maxLifetime + 1`, so a steady block's live count is its capacity minus about one and there is nothing worth compacting. Step the slots in place, write a zero `Size` for a dead one so its quad has no extent, and draw the whole capacity - no atomics, no second pool, no indirect draw, and the CPU still knows every draw range
- [_] particle slots become a ring: with nothing compacting, `First + Spawned % Capacity` picks the newborn's slot and it is the right rule because particles die in age order. **A behaviour change**: a full block drops the spawn today and a ring overwrites the oldest, which for a ranged lifetime can take something still alive
- [_] particle buffers as `uint words[]` indexed by hand, because `vec3` takes sixteen-byte alignment in std430 and neither `ParticleState` (40 bytes) nor `ParticleInstance` (28) is laid out that way - the same thing `occlusion-args.comp` does with its five-word draw command. Instances are a compute storage buffer *and* a vertex stream, which the occlusion cull already pairs
- [_] particle spawning stays on the host because it reads `ParticleEmitter`; about 1,700 births a tick at 68 bytes is 115 KB against the 16 MB the whole pool costs, scattered into their ring slots by a small compute pass
- [_] portal seam carrying for particles: `client::CarryThroughSeams` reads and rewrites particle positions on the host so a spark carried into a doorway arrives in the far room. With the pool on the device it has to move into the step shader with the seam list uploaded beside the blocks, or it stops working
- [_] retire the host-side particle step tests: `engine.effects.particles` asserts where a particle ended up against the host pool and there is no device in a test binary. The emitter, block, free-list and spawn-plan half survives. A headless server stops simulating particles, which is consistent - it never drew them
