...
Progress: client, server and Studio expose isolated-world lifecycle and versioned discovery; typed Python and independent MCP callers agree on thirteen capture planes, and named multicamera capture shares one renderer frame. The final rebuilt release Vulkan default headless run completed a 13-plane capture and retired its world. Live typed versus independent MCP parity had zero mismatches; `first_surface_validity` was 921,600 bytes with only 0 and 255 values, and typed and raw bytes were identical. The default headless data-factory now paces at 60 FPS; at 11 seconds, idle CPU was about 8.7% versus about 100% uncapped, with a longer capped sample at about 3.3%. The Python sibling full suite passed 606 tests. Full control passed 116 cases and 2,342 assertions; full script passed 123 cases and 2,317 assertions. The ledger refuses fresh IDs at 256 while old IDs replay, and focused Surface mutation refusal passed. The broad render suite passed all 105 cases and 1,690,393 assertions.
...
- [x] Expose bounded `raw-scene/v2` MCP extraction with portable little-endian mesh sections, original source texture bytes, authored material facts, bounded point lights, stable IDs, revision fences and BLAKE3-verified ranged reads. The typed Python reader and a direct MCP caller returned identical bytes from a live headless Vulkan world with an EditableImage map. The first resource remains capped at 320 MiB.
- [x] Convert validated `raw-scene/v2` extracts to a bounded glTF 2.0 GLB subset in sibling `datafactories-docs/raw_scene_gltf.py`. The live `export_raw_scene_gltf_live.py` command creates and retires its owned world, snapshots and fences the raw extract, and emits a verified five-file bundle carrying `engine_version` and `snapshot_id`. The release demo produced a 5,116-byte GLB with five nodes, three meshes, one camera, one point light and seven explicit losses. Bounded point lights use `KHR_lights_punctual`; engine brightness maps to KHR candela approximately and remains an explicit loss. Khronos validation reported 0 errors, warnings and infos; default loss policy refuses and `--allow-losses` succeeds. Python full suite passed 606 tests after coverage reuse. The writer stages five files in a sibling directory, fsyncs each file and the staging directory, uses Linux `renameat2(RENAME_NOREPLACE)` or a Windows no-replace rename, and refuses safely on unsupported POSIX targets before fsyncing the parent. The concurrent-writer test passed its 16-test suite. A parent-sync failure can leave a published bundle whose durability is unconfirmed. The full external glTF profile remains open.

Raw spatial extraction:

- [x] Keep the raw spatial boundary extract-only. Focused engine control coverage exercises the `raw-scene/v2` consumer flow: begin a revision-fenced extract, read the resource through bounded ranges, verify its BLAKE3 hash, release it, and reject stale reads after a world revision change. Each range is capped at 1 MiB and each resource at 320 MiB.
