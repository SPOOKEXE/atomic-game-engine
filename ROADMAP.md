...
- [x] expose the DataSceneOptions base slice through nested MCP `capture_bundle` options. It validates the complete `data-scene-options/v1` object, bounded strings, duplicate channels and paired second-surface planes, preserves the exact tick, epoch, version and operation guards, and reuses capture polling, cancellation, release and retained-resource lifecycle.
