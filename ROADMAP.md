...
- [_] deferred `D00108` — a shared document for team create. `replication::Authority` orders a server's world and not two people's undo stacks, and inventing one to make the panel look finished is exactly the half-added feature AGENTS.md refuses. The session layer above is what it will arrive on.
