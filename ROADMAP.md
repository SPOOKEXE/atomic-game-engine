...
- [_] can we review the client, server and studio code and merge what we can into the engine layer instead? I notice some functions would be better off in the engine instead of these mono repositories. Also ensure studio and clients use the same render pipelines and whatnot, create one location for all rendering, and same for similar other behavior (maybe even setup singletons, interfaces or such for these behaviors).
