...
- [_] expose PBR albedo, roughness, metallic, emissive, specular, transmission, shading geometry, normals and UV maps. One aligned capture now exports authored sRGB albedo, the raw packed material attachment, linear HDR emissive, packed shading normals and perspective-correct authored mesh UVs through MCP and Luau. Mesh UV is RG16F with finite visible built-in opaque or masked fragments and NaN for background, discarded alpha or custom-shader geometry. The built-in material decode is named R roughness, G metalness, B material occlusion and A reserved; the raw attachment remains authoritative. Eight-target G-buffer support is established by successful native pipeline creation, with an automatic Tier C fallback when unavailable. Specular and transmission return explicit unsupported provenance because the current material model has no separate authored facts. Complete shading-geometry records and Python artifact example coverage for UVs remain open.
