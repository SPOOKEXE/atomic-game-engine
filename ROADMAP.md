...
- [_] expose PBR albedo, roughness, metallic, emissive, specular, transmission, shading geometry, normals and UV maps. One aligned capture now exports authored sRGB albedo, the raw packed material attachment, linear HDR emissive and packed shading normals through MCP and the typed Python client. The example saves each as a viewable PNG while preserving raw type, shape, stride, origin, statistics and checksums in JSON. Named roughness and metallic decoding, specular, transmission, UVs and complete shading-geometry records remain open.
