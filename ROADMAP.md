...
- [_] deferred `D00108` — a shared document for team create. **Two of the three pieces are now in.** The unit is settled — a committed waypoint, handed over whole by `CommandLog::Watcher::Committed` — and so is the far end: `ApplyForeign` lands somebody else's waypoint without it entering this author's undo stack, and `Adopt` is what makes a foreign `EditId` resolvable locally. What is missing is the middle: an identity two editors share, since an `EditId` is one log's own name for an instance, and a reliable ordered bus to carry the bytes. `replication::Listener` and `Connector` already own a connected session and a reliable channel, so the bus is a widening of those rather than a fourth session type — and building a fourth is the thing to refuse. The `Committed` seam is deliberately hooked to nothing rather than to a stub.
