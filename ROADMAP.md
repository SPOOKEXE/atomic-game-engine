...
- [_] define MCP idempotency, expected versions, structured status, cancellation, capability limits, permissions and audit records. One surface-owned ledger now applies the same tool plus normalized-argument replay rule across world lifecycle mutations, capture submission, nested `capture_bundle`, cancellation, release and script packages. Reusing an operation ID for another tool or argument is rejected before side effects. The newest 256 redacted audit rows expose tool, operation ID, terminal status and refusal state without raw arguments; an evicted ID becomes fresh. Expected-version rules for read-only observations, authentication, permissions and cancellation across every asynchronous tool remain open.
