...
- [_] `ShapeCast` still asks the volume walk for the union of where a swept box starts and ends, which is loose for a long sweep the same way a raycast's segment bounds were. A swept box is a line with thickness, so it needs its own walk and its own de-duplication rule - the ray walk's run rule holds for the line alone. Nothing has measured it as hot, which is why it is here rather than done.
