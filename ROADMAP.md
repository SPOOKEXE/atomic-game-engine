...
- [x] optimise particles and physics
- [x] GPU-resident particles: the pool lives on the device and the step is a compute pass. The frame's two remaining particle costs were both the CPU touching every particle - `particles.pack` copied the whole live pool to a staging buffer and `particles.age` simulated it, 0.83 ms and 1.0 ms in `release` at 512,000 particles against 0.02 ms to record the draw. Both spans are gone; `particles.step` records and submits the dispatch in 0.02 ms. **Measured `release`, `--uncapped`, against the commit before, both runs long enough to reach a steady particle count and both drawing the same picture to within a fifth of a percent of lit pixels: 5,120 emitters and 512,000 particles goes 157.2 fps to 1,098.6, and the roadmap's 102,400 emitters goes 58.1 to 98.8.** Peak memory is within sixteen megabytes either way: the device pool, the two block tables and the staging cost about what releasing the host's own instance and state arrays returns. Fill was never a factor, so screen-space splatting would have solved a bottleneck this engine does not have. `mono.engine/effects` is a `shared` tier with no device, so the step is in `render`; effects keeps the emitters, blocks, free lists and spawn plans, and keeps its host-side pass as the reference the tests assert against
- [x] particle step in compute with no compaction: `BlockSizeFor` is `Rate * maxLifetime + 1`, so a steady block's live count is its capacity minus about one and there is nothing worth compacting. Step the slots in place, write a zero `Size` for a dead one so its quad has no extent, and draw the whole capacity - no atomics, no second pool, no indirect draw, and the CPU still knows every draw range
- [x] particle slots become a ring: with nothing compacting, `First + Spawned % Capacity` picks the newborn's slot and it is the right rule because particles die in age order. **A behaviour change**: a full block drops the spawn today and a ring overwrites the oldest, which for a ranged lifetime can take something still alive
- [x] particle buffers as `uint words[]` indexed by hand, because `vec3` takes sixteen-byte alignment in std430 and neither `ParticleState` (now 60 bytes) nor `ParticleInstance` (28) is laid out that way - the same thing `occlusion-args.comp` does with its five-word draw command. Instances are a compute storage buffer *and* a vertex stream, which the occlusion cull already pairs. **`Position` and the accumulated turn moved from the instance into the state**, which is what makes the instance stream pure output: the step writes it straight to the sorted draw range rather than to a pool something then has to gather, so the twenty draw calls the state sort buys are kept with no second pass
- [x] particle spawning stays on the host because it reads `ParticleEmitter`; about 1,700 births a tick at 64 bytes is 109 KB against the 16 MB the whole pool cost, scattered into their ring slots by `particle-spawn.comp`
- [x] portal seam carrying for particles: `client::CarryThroughSeams` reads and rewrites particle positions on the host so a spark carried into a doorway arrives in the far room. With the pool on the device it has to move into the step shader with the seam list uploaded beside the blocks, or it stops working
- [x] a recycled particle block starts empty without being cleared: the device draws a block's whole capacity, so rows handed to a new emitter would draw the last one's particles with the new one's curves for as long as their lifetimes had left. `EmitterBlock::Generation` is bumped when the rows are taken and a particle carries the number it was born under, so the step reads a mismatch as death - against uploading a zeroed state for every row of every block claimed, which at scene load is the whole pool
- [x] the particle block record splits by how often it changes: the first device-resident version staged a whole 384-byte record for every emitter every frame, which at 102,400 emitters is 39 MB of writes into a mapped transfer buffer - *more* than the 16 MB the entire pool cost before it moved, so the move was a loss at exactly the scale it exists for. The curves change when a script writes the emitter and the parameters when it moves, so both become tables indexed by block written only where `EmitterBlock::Revision` disagrees with what the device was last told; what crosses per emitter per frame is the two words a draw needs. `particle-scatter.comp` sends the difference, and a settled scene sends nothing
- [x] the particle update staging is a fixed 2,048 rows: it is cycled every frame, so its *size* is what a frame costs whether or not anything is in it. Sized to the emitter count it was 34 MB re-versioned sixty times a second, which ran 102,400 emitters at 4 fps and held hundreds of megabytes of transfer-buffer versions waiting on fences. A block whose turn has not come is left out of the draw list rather than drawn from a table that does not yet describe it
- [x] a device-stepped pool allocates neither host array: `ParticleSystem::Births` carries the whole newborn state rather than a row into `States`, so nothing on the host side ever reads a particle again and `StepParticles` releases `Instances` and `States` on its first such tick - 54 MB at the client's default capacity that nothing would ever look at
- [x] a device block empties by arithmetic: the host cannot see a particle die, so `Live` is what it has ever put in the block and never falls - a disabled emitter would hold its rows and go on drawing its capacity in quads with no extent. `EmitterBlock::Idle` counts seconds since the last birth and past `Longest` nothing born before can still be alive
- [x] retire the host-side particle step tests - **not done, and the reason is better than the plan was.** `mono.engine/effects` is `shared` and only `mono.client` ever installed a pool, so nothing but the client needed the host-side pass to go; keeping it costs one branch on `ParticleSystem::DeviceStepped` and buys the reference the compute shader is written against, which is the only executable statement of what a particle is supposed to do. Four tests were added for what the device path does differently instead: births reaching the right rows, the ring going round rather than refusing, a recycled block disagreeing with what it inherited, and `Live` reaching zero from arithmetic once nothing is born
