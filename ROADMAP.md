...
- [_] trace and fix the intermittent portal product walk failures. Found and fixed on 2026-09-25: forwarded input drained one row per world tick, so a 60 Hz client fell half a tick behind a 30 Hz world every tick; forwarding closed when native input was queued rather than applied, so the body kept a stale direction for up to the forward lag; a grounded body's destination suffix stopped on its own floor and landed on the pane plane; the walk steered around a point 1.7 m past the return pane until adoption and could walk back through it; stage 1 steered before it settled; eye height was checked on cleared-camera frames. The strict 30/60 walk now passes 18 of 20. Still open: after any crossing the destination applies input about 0.5 s behind the client, because the source freezes the body during prepare and the destination replays that backlog in real time, then anchors native input to it (`[portal-input-schedule]` preserves this by design). While that lag lasts, a body predicted beyond the pane shows the source room until the server confirms (line 1056, about 1 in 10). Fix the handoff so the committed body catches up to the client's input clock, then rerun the strict 30/60 walk, the 144/240 FPS, stall and fault walks, and the impairment grid until each passes repeatedly. The fault round trip fails at HEAD with or without these changes (black eye frames, one adoption).
...
- [_] add typed physics and replication observation hooks with an optional trace id. Portal debugging on 2026-09-25 parsed trace log lines and capture sidecars to follow one crossing through two worlds and a client, and enabling those logs changed the timing. Hooks replace that with bounded typed records.
