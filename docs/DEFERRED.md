...
- **The `bench` and `release` presets compile at `-O2`, and one measured loop is 2.4x faster at `-O3`.** Found while rejecting v0.4's vectorisable-component-layout item: 100k rows of the iteration control run 38.74 us at `-O2` and **16.61 us at `-O3`**; 500k run 196.38 and **85.63**. Same code, same machine, same quiet conditions.
- **This is written down because of what it does to every other number in the repository.** `ROADMAP.md` carries a lot of measurements, and a standing discipline that an algorithmic change needs a number attached. All of those numbers were taken at `-O2`. A 2.4x floor sitting under them does not make any of them wrong, but it does mean **the ratio between two implementations can invert with the flag** — which is the exact hazard the `bench` preset's own comment already warns about for debug builds, one level up.
- It also nearly caused a wrong decision: the layout item's premise was that a twelve-byte stride cannot be vectorised cleanly and needs padding to sixteen. At `-O3` the packed layout is 2.4x the padded one, because a twelve-byte AoS of floats has no gaps and the vectoriser fills every lane, while padding puts a dead lane in every vector. **Measured at `-O2` alone, both layouts look the same and the item looks merely unhelpful rather than backwards.**
- Not acted on, deliberately: changing what `release` means has a blast radius across the whole roadmap's evidence, and `-O3` is not free — it trades size and compile time, and it can change floating-point behaviour, which for a repository whose determinism recipe diffs two runs byte for byte is a question to answer before flipping a flag, not after.
- **Reopen trigger: the next time somebody proposes a data-layout change for speed.** This is the cheaper thing to try first, and it should be ruled in or out before `Column` is reopened again. The measurement to take is `just determinism` and `just replay-check` at `-O3` — if those stay byte-identical, the objection is only size and compile time.
