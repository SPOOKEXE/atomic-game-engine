...
| `scene.GpuParticleField` | 12 | 4 | yes | . | . | . | Authored request for a deterministic analytical storm particle field, including its enabled state, visible layers, normalized count request, and reset seed. |
...
| `scene.LightingService` | 104 | 4 | yes | yes | . | . | On the single `Lighting` service instance: ambient and outdoor ambient colour, fog colour and range, sun brightness, time of day and geographic latitude. |
...
| `scene.RigidBody` | 40 | 4 | yes | yes | . | . | Mass, linear and angular damping, and body kind for a physics body. Gravity queries it every tick and the contact solver reads it per contact. |
...
| `scene.SurfaceAppearance` | 88 | 4 | yes | . | . | . | The seven texture maps, shader name, alpha mode and cutoff a drawable is rendered with. `ResolveMaterials` writes it and the PBR paths read it. |
...
| `script.EventNarratives` | 152 | 8 | yes | . | . | . | Per-world canonical bundle of validated script-declared narrative records, retained for data-scene reads and checkpoint serialization. |
...
231 components registered by the engine, 0 without a purpose line.
