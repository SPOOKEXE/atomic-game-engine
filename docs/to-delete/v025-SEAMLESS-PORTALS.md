...
### Implementation and acceptance status, 25 September 2026

The acceptance matrix now runs end to end. `just portal-product-acceptance` runs the product walk at 30 and 60 Hz world rates, at 144 and 240 FPS presentation with 60 Hz worlds, under a variable frame and stall schedule whose 0.2 s stalls land inside both handoffs, and across the 24-cell grid of 0, 50, 150 and 300 ms RTT, 0 and 30 ms jitter, and 0, 1 and 5 percent loss, with 1 percent duplicate and reorder injection in every cell. Impairment is applied where the client's sockets receive, from a seed, and each cell proves its faults reached the client. The runner also takes matched seam captures at 1080p and 4K and uncaptured `--frame-timings` series, and records failing cells instead of stopping. The earlier 60.58 FPS reading for the 240 FPS row measured capture I/O; without capture the seam scene achieves 240.0 FPS at 1080p, with a p99 frame interval of 4.43 ms and a p99 summed GPU pass time of 0.37 ms.

The grid found a latency livelock, now fixed. A camera route that started before the replica learned its authored name recorded the local name. Once the identity arrived the route no longer resolved, so every frame lost its camera world, drew nothing, and requested an eye for an empty destination, which removed that eye's source and retired its request in flight. At 150 ms RTT or more no eye reply could land, so the handoff never committed. The camera resolver now maps a replica's local name to its authored name, and an unresolved destination is no longer submitted. After the fix every 150 ms cell adopts.

A frame whose demanded destination image has not arrived now reports `PortalPresentation::AwaitingImage` in the capture sidecar. Impaired walks accept a missing body only on such frames and count them per cell, so a late image is a named degraded state rather than a silent gap.

The matrix does not pass. The product walks are flaky with and without impairment. The strict 30/60 handoff failed 3 of 3 runs on the code from before the 25 September portal changes, so the single 24 September pass (`/tmp/atomic-portal-product-final-1-NkxJ4k`, 21,853 assertions) does not reproduce. The clearest failure is a third crossing: after the return, the body rests just behind the source pane and is transferred back about 44 frames later without moving. The 24 September run also ended behind the pane, and simply did not re-cross before its frames ran out. Diagnostic logging hides the race. The remaining grid failures are black frames, single adoptions at high RTT and camera checks. Tracing and fixing this flakiness is a v0.26 roadmap item. This plan is not accepted until those walks pass repeatedly.
...
`PortalSeam.luau` is the focused visual and network scene. `ImmersivePortals.luau` is the longer interactive integration scene. `scripts/demos/capture-portal-seam.sh` and `portal-seam-report.py` compare temporal matched-room frames, including uncovered and duplicate body samples. The status section above records what the matrix runner covers and where it still fails. Each report should name the fixture, resolution, backend, pipeline revision, presentation time, topology revision, and tolerance.
