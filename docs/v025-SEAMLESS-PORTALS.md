...
| Body draw splitting | [`PortalGeometryDraw.cpp`](../mono.engine/render/src/PortalGeometryDraw.cpp) can derive near and far draw halves from current rows and one skin palette, with complementary cuts and far tag suppression. It supports skinned palettes but has current surface and variant limits. | The draw primitive is useful, but it must be wired into production draw submission and expanded to the required drawable set. |
...
| Production portal geometry | `SplitPortalBodyDraws` currently has test callers only. `PortalImageRuntime` appends wire geometry through `AppendPortalDraws`; `ForwardPortalDraws` forwards nested demand geometry. | Production wiring is a first delivery stage, not a polish item. |
...
| Physics | [`Portals.cpp`](../mono.engine/physics/src/Portals.cpp) creates bounded same-world proxy bodies for static foreign colliders. It has no cross-world dynamic island solver and no copied physical material. | Static collision bridging is a starting point only. Dynamic contacts across a seam require a single deterministic solve. |
...
`PortalSeam.luau` is the focused visual and network scene. `ImmersivePortals.luau` is the longer interactive integration scene. The existing capture scripts, `scripts/demos/capture-portal-seam.sh` and `portal-seam-report.py`, currently measure a still side ratio. Extend them with a temporal full-reference sequence before using them to assess a moving front seam. The report should name the fixture, resolution, backend, pipeline revision, presentation time, topology revision, and tolerance.
...
The normal code gates are the narrow module suites, `just preset=ci check`, `just test-architecture`, `just source-check`, and `just em-dash-check`, with suite filters verified from their IDs before use. New benchmark recipes belong in `just` and write only under the build output location. A final headless GPU or live Studio verification requires user approval under the repository instructions. Until that approval and those runs occur, this document makes no visual or performance pass claim.
