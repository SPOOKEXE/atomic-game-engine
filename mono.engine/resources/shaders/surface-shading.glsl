...
layout(set = 3, binding = 0) uniform Lighting {
	...
	//    recursion**, which has no image to read and shades flat; 4 projects an
	//    imported HDR portal image without a mirror tint. See
...
void shadeSurface() {
	...
	vec3 emissive = (features & FEATURE_EMISSION) != 0u
		? inEmission.rgb * inEmission.a *
			(lighting.Material.w > 0.5 ? texture(emissiveMap, cellUv).rgb : vec3(1.0))
		: vec3(0.0);
	...
	if (lighting.Flags.z > 2.5 && lighting.Flags.z < 3.5) {
	...
	if (lighting.Flags.z > 3.5 && onPane) {
		if (inSurfacePosition.w > 0.0) {
			vec2 projected = inSurfacePosition.xy / inSurfacePosition.w;
			vec2 uv = vec2(projected.x * 0.5 + 0.5, 0.5 - projected.y * 0.5);
			if (all(greaterThanEqual(uv, vec2(0.0))) && all(lessThanEqual(uv, vec2(1.0)))) {
				vec3 image = texture(surfaceMap, uv).rgb;
				outColour = vec4(Encode(mix(lit, image, lighting.Flags.w)), max(alpha, lighting.Flags.w));
				return;
			}
		}
		outColour = vec4(Encode(lighting.Ambient.rgb), max(alpha, lighting.Flags.w));
	...
	if (lighting.Flags.z > 1.5 && lighting.Flags.z < 2.5 && onPane) {
	...
	if (lighting.Flags.z > 0.5 && lighting.Flags.z < 1.5) {
