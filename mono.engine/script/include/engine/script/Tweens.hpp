...
namespace engine::script {
	...
	struct TweenGoal {
		...
		// Which blend `Interpolate` runs over the two buffers, and the answer
		// `Interpolable` refused a non-numeric property by. Captured with the
		// goal for `Property`'s reason: the descriptor is not this module's to
		// hold on to.
