...
namespace engine::script {
	...
	enum class CodecStatus : uint8_t {
		...
		// Two of a map's entries have the same key.
		//
		// **A table can produce one**, which is why this exists: Lua's `[1]` and
		// `["1"]` are two keys and both spell `"1"` once a key becomes a name.
		// Whichever entry the sort put second would win, so the value a caller
		// gets back would depend on the order the VM happened to walk the table
		// in. Saying so is the only answer that does not lose one of the two
		// silently.
		//
		// @since v0.19
		DuplicateKey,
	...
	// Puts one map's entries in the order the format writes them.
	//
	// **The sort is the determinism guarantee, and it lives here so that there
	// is one of it.** `Encode` writes a bus payload and `HttpService` writes a
	// JSON document, and the two have to agree about the order or one value
	// crosses a bus in one shape and lands in a document in another. §1 above is
	// the argument for sorting at all; this is where it happens.
	//
	// By key bytes, which `std::string`'s own comparison already is: its traits
	// order `char` as `unsigned char`, so `"e"` before `"\xC3\xA9"` whichever
	// VM built the table.
	//
	// @param entries The entries, reordered in place.
	// @return `Ok`, or `DuplicateKey` naming the one thing a sorted run can
	//         reveal that an unsorted one cannot.
	//
	// @since v0.19
	...
	//
	// Sorts every map's entries with `SortEntries` on the way, in place, so the
