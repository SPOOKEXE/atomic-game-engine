...
namespace engine::script {
	...
	namespace {
		// Puts a constant vector on a constructor table that `InstallValueType` has just
		// set as a global.
		//
		...
		// behind `Vector3.zero` cannot be written through. `InstallValueType` seals the
	...
	void OpenValues(lua_State *state) {
		...
		InstallValueType(state, "Color3", Color3Index, nullptr, colorConstructors, nullptr, Color3Equal);
		...
		*PushCFrame(state) = CFrame();
		...
		//
		// After both `InstallValueType` calls, because the global table they set is what
		// these attach to.
