...
namespace engine::render {
	...
	struct AdornmentLine {
		// The entity that owns this line.
		ecs::Entity Source;
		...
		// Whether pointer interaction is enabled for this line.
		bool Interactive = false;
	};

	// A ray hit on an interactive adornment.
	struct AdornmentHit {
		// The entity whose adornment was hit.
		ecs::Entity Source;
	...
	struct AdornmentPointer {
		// The world-space ray under the pointer.
		core::Ray Ray;
		// The pointer position in the host view.
		core::Vector2 Position;
		// Whether the primary button is held.
		bool PrimaryDown = false;
		// Whether the secondary button is held.
		bool SecondaryDown = false;
		// Whether the pointer moved since the previous update.
		bool Moved = false;
		// Whether the pointer is inside the host view.
		bool Inside = true;
	};
	...
	class AdornmentGeometry {
	  public:
		...
		// Finds the nearest interactive adornment intersecting a ray.
		//
		// @param ray The world-space ray to test.
		// @param radius The hit radius around each line.
		// @return The nearest hit, or no value when the ray misses.
	...
	class AdornmentPointerRouter {
	  public:
		// Converts pointer state into script-facing adornment events.
		//
		// @param store The world containing adornments.
		// @param pointer The current host-provided pointer state.
		// @param radius The hit radius used for pointer capture.
		// @return Events generated by this update, valid until the next update.
		std::span<const gui::GuiEvent>
		Update(ecs::Store &store, const AdornmentPointer &pointer, float radius);

		// Releases any active pointer captures and clears pending state.
		void Forget();
