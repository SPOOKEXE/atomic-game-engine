...
TEST_SUITE_ID("engine.scripthost.runtime")
...
//
// Rule 5: work inside a tick may be parallel, work across ticks may not. A
// script that can run forever inside one tick has taken the tick boundary away,
// and both ways of arranging that are here - a queue that refills itself, and a
// budget asked for a second time.

namespace {
	// Kills the process if the scope it guards has not ended in time.
	//
	// **A wall clock in the test and nowhere near the engine.** What the cases
	// below check is counted and deterministic; this only decides how a
	// regression presents. Without it an unbounded drain hangs the whole suite
	// with no output, which is the least useful failure a test can produce.
	//
	// **The work stays on the calling thread and only the timer moves off it**,
	// which is not a style choice: `ecs::Store` refuses a touch from a thread
	// that does not own it, and QuickJS measures its call depth against the
	// stack address its context was made on. A VM driven from a borrowed thread
	// fails for both reasons before it reaches anything this file is about.
	//
	// Aborts rather than failing: the guarded thread is still inside a VM, so
	// there is nothing to unwind to and nothing to report from.
	class Watchdog {
	  public:
		explicit Watchdog(std::chrono::seconds limit)
			: Timer([this, limit] {
				  std::unique_lock<std::mutex> held(Gate);
				  if (Woken.wait_for(held, limit, [this] { return Done; })) {
					  return;
				  }
				  std::fputs("the script host did not return: a call into a VM is unbounded\n", stderr);
				  std::fflush(stderr);
				  std::abort();
			  }) {}
		...
		~Watchdog() {
			{
				const std::lock_guard<std::mutex> held(Gate);
				Done = true;
			}
			Woken.notify_all();
			Timer.join();
	...
	// A microtask that queues a microtask. The queue never empties.
	constexpr const char *ENDLESS_MICROTASKS = R"(
		const again = () => { Promise.resolve().then(again); };
		again();
	)";
}

TEST_CASE("a microtask that queues itself is cut off rather than hanging the tick", "[script][js]") {
	RegisterClasses();
	Store store("script_test");

	RuntimeLimits limits;
	limits.JobBudget = 4096;

	// The step budget cannot see this and no smaller one would: QuickJS polls
	// the interrupt handler once per ten thousand safepoints, and each of these
	// jobs is a handful of them. Measured against the vendored VM, the drain
	// reached 52 million jobs and 31,201 polls in two minutes - so the default
	// budget of 200 million polls would be spent some time in the next twenty
	// years, and the queue is not draining in the meantime.
	const auto runtime = MakeRuntime(store, Language::JavaScript, limits);

	bool ran = true;
	{
		const Watchdog watchdog(std::chrono::seconds(60));
		ran = runtime->Run(ENDLESS_MICROTASKS);
	}

	CHECK_FALSE(ran);
	CHECK(runtime->LastError().find("microtask budget") != std::string::npos);
}

TEST_CASE("the default microtask bound also terminates", "[script][js]") {
	RegisterClasses();
	Store store("script_test");

	// The bound a host that sets nothing gets. A default large enough to be
	// indistinguishable from no bound would leave every real program in the
	// state this case exists to rule out.
	const auto runtime = MakeRuntime(store, Language::JavaScript);
...
TEST_CASE("a beat whose reactions never end is still bounded", "[script][js]") {
	RegisterClasses();
	Store store("script_test");
	...
	// The beat drains too, so the same queue can hang the same host from the
	// other entry point. Two beats, because the bound has to hold on each one
	// rather than only on the first - the queue is still full when the second
	// starts, which is what makes the cost per tick the thing that is bounded.
	const auto runtime = MakeRuntime(store, Language::JavaScript, limits);
	REQUIRE(runtime->Run(R"(
		const again = () => { Promise.resolve().then(again); };
		RunService.Heartbeat.Connect(() => { again(); });
	)"));
	...
	{
		const Watchdog watchdog(std::chrono::seconds(60));
		first = runtime->Heartbeat(0.016f);
		second = runtime->Heartbeat(0.016f);
	}
...
TEST_CASE("a step budget is spent once, not once per pcall", "[script]") {
	RegisterClasses();
	Store store("script_test");
	...
	// A counter zeroed when it trips hands the next `pcall` a whole fresh
	// budget, so this script catches five refusals and finishes - a script
	// running as long as it likes, one trip at a time. A budget spent once
	// refuses the first loop and every safepoint after it.
	const auto runtime = MakeRuntime(store, Language::Luau, limits);
...
TEST_CASE("what a script spent survives the budget it blew", "[script][js]") {
	RegisterClasses();
	Store store("script_test");

	RuntimeLimits luauLimits;
	luauLimits.StepBudget = 100000;

	// **The script that used the most is the one a profile panel is opened to
	// find**, and it was the one the old counter reported nothing for: zeroing
	// on the trip threw the whole figure away.
	const auto luau = MakeRuntime(store, Language::Luau, luauLimits);
	CHECK_FALSE(luau->Run("while true do end"));
	CHECK(luau->StepsTaken() >= luauLimits.StepBudget);

	// A far smaller number, because one QuickJS step is ten thousand safepoints
	// and a hundred thousand of them is a spin measured in seconds. The claim is
	// that the figure exists and moves, not that it matches Luau's.
	RuntimeLimits jsLimits;
	jsLimits.StepBudget = 200;

	const auto javascript = MakeRuntime(store, Language::JavaScript, jsLimits);
	const uint64_t before = javascript->StepsTaken();
	CHECK_FALSE(javascript->Run("while (true) {}"));
	CHECK(javascript->StepsTaken() >= before + jsLimits.StepBudget);
}
