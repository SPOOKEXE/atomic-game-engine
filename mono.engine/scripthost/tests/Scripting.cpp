...
TEST_SUITE_ID("engine.scripthost.scripting")
...
TEST_CASE("CurrentCamera refuses something that is not a camera", "[scripting]") {
	// Every consumer resolves its own matrices from the row `ActiveCamera` names
	// and gives up on a row with no `Camera`, so accepting one would present as a
	// view that stopped following anything with nothing reporting why.
	RegisterClasses();
	Store store("script_test");
	const auto runtime = MakeRuntime(store, Language::Luau);
