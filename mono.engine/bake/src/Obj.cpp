...
namespace engine::bake {
	...
	bool ReadObj(std::span<const std::byte> bytes, ImportedModel &out, std::string &failure) {
		...
		while (lineStart <= text.size()) {
			...
			} else if (fields[0] == "mtllib" && fields.size() >= 2) {
				// **Recorded and not opened.** This module has no filesystem -
				// `ImportedModel::Textures` is a list of spellings for the same
				// reason - so the name goes out with the model and whoever knows
				// where the model came from reads it. `assetc` does.
				//
				// The first one wins. An `.obj` may name several libraries and
				// the ones that do are generated by exporters this engine has
				// not met; taking the first is a stated rule rather than a
