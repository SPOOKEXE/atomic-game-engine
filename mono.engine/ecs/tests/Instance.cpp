...
TEST_CASE("the instance components are registered under explicit names", "[ecs][instance]") {
	// **A file, a wire and a recording all carry these strings**, so renaming
	// one is a format change and this is what makes it show up as a failing
	// test rather than as a snapshot that loads into a world where every
	// instance has lost its name. `scene/tests/Registration.cpp` keeps the same
	// list for the same reason.
	//
	// They were not always explicit. `Hierarchy` was named by `scene`, one
	// module up, and the other two by `Components::Of<T>` — which spells the
	// type the way the compiler happens to, so the name in the file depended on
	// what built the program.
	Store store("registration");

	for (const std::string_view expected : {"ecs.InstanceClass", "ecs.Hierarchy", "ecs.InstanceName"}) {
		INFO(expected);
		REQUIRE(Components::Find(Name(expected)).IsValid());
	}
	...
	REQUIRE_FALSE(Components::Find(Name("InstanceName")).IsValid());
}

TEST_CASE("an instance name crosses a snapshot as text", "[ecs][instance]") {
	// `InstanceName` holds a `core::Name`, whose id is first-seen order within
	// one process — so the generated byte serialiser was writing an interning
	// counter into the file, and a world reloaded by a process that had
	// interned its strings in another order came back with every instance
	...
	const Entity child = source.CreateInstance(tree.Part, "Wheel");
	const Entity anonymous = source.CreateInstance(tree.Part);
	source.SetParent(child, model);
	source.SetParent(anonymous, model);

	ByteWriter writer;
	REQUIRE(source.Save(writer));

	Store restored("name-restored");
	ByteReader reader(writer.Bytes());
	REQUIRE(restored.Load(reader));

	REQUIRE(restored.InstanceNameOf(model) == Name("Chassis"));
...
TEST_CASE("descendants come back in the order a recursive walk would", "[ecs][instance]") {
	...
	const Entity first = store.CreateInstance(tree.Model, "First");
	const Entity firstLeaf = store.CreateInstance(tree.Part, "FirstLeaf");
	const Entity second = store.CreateInstance(tree.Model, "Second");
	const Entity secondLeaf = store.CreateInstance(tree.Part, "SecondLeaf");
	store.SetParent(first, root);
	store.SetParent(firstLeaf, first);
	store.SetParent(second, root);
	store.SetParent(secondLeaf, second);

	std::vector<Entity> seen;
	store.EachDescendant(root, [&](Entity descendant) { seen.push_back(descendant); });

	// The root itself is not one of its own descendants.
	REQUIRE(seen == std::vector<Entity>{first, firstLeaf, second, secondLeaf});

	std::vector<Entity> none;
	store.EachDescendant(firstLeaf, [&](Entity descendant) { none.push_back(descendant); });
	CHECK(none.empty());
}

TEST_CASE("a recursive FindFirstChild answers the nearest match", "[ecs][instance]") {
	...
	const Entity root = store.CreateInstance(tree.Model, "Root");
	const Entity branch = store.CreateInstance(tree.Model, "Branch");
	const Entity buried = store.CreateInstance(tree.Part, "Target");
	const Entity shallow = store.CreateInstance(tree.Part, "Target");
	store.SetParent(branch, root);
	store.SetParent(buried, branch);
	store.SetParent(shallow, root);

	// `branch` was parented first, so a plain depth-first walk reaches `buried`
	// before `shallow`. The child pass has to win.
	CHECK(store.FindFirstChild(root, "Target", true) == shallow);
	CHECK(store.FindFirstChild(root, "Target") == shallow);

	// And with the shallow one gone, the deep one is found only recursively.
	store.DestroyInstance(shallow);
	CHECK(store.FindFirstChild(root, "Target") == NULL_ENTITY);
	CHECK(store.FindFirstChild(root, "Target", true) == buried);
	CHECK(store.FindFirstChild(root, "Absent", true) == NULL_ENTITY);
	CHECK(store.FindFirstChild(root, "", true) == NULL_ENTITY);
}
...
TEST_CASE("OfClass is exact and WhichIsA is not", "[ecs][instance]") {
	...
	const Entity part = store.CreateInstance(tree.Part, "Part");
	const Entity nested = store.CreateInstance(tree.Part, "Nested");
	store.SetParent(part, root);
	store.SetParent(nested, part);

	CHECK(store.FindFirstChildOfClass(root, tree.Part) == part);
	CHECK(store.FindFirstChildOfClass(root, tree.BasePart) == NULL_ENTITY);
	CHECK(store.FindFirstChildOfClass(root, tree.Model) == NULL_ENTITY);
	CHECK(store.FindFirstChildOfClass(root, ClassId{}) == NULL_ENTITY);

	CHECK(store.FindFirstChildWhichIsA(root, tree.BasePart) == part);
	CHECK(store.FindFirstChildWhichIsA(root, tree.Instance) == part);
	CHECK(store.FindFirstChildWhichIsA(root, tree.Model) == NULL_ENTITY);

	// `nested` is a grandchild, so only the recursive form reaches it.
	CHECK(store.FindFirstChildWhichIsA(part, tree.BasePart) == nested);
	CHECK(store.FindFirstChildWhichIsA(root, tree.Model, true) == NULL_ENTITY);
}
...
TEST_CASE("ancestors are searched upwards and exclude the instance", "[ecs][instance]") {
	...
	CHECK(store.FindFirstAncestor(leaf, "Middle") == middle);
	CHECK(store.FindFirstAncestor(leaf, "Top") == top);

	// **Not itself.** `FindFirstAncestor("Leaf")` from `Leaf` is nothing, or
	// every instance would be its own ancestor and the walk would never be
	// asking a question.
	CHECK(store.FindFirstAncestor(leaf, "Leaf") == NULL_ENTITY);
	CHECK(store.FindFirstAncestor(top, "Top") == NULL_ENTITY);
	CHECK(store.FindFirstAncestor(leaf, "Absent") == NULL_ENTITY);
	CHECK(store.FindFirstAncestor(leaf, "") == NULL_ENTITY);

	CHECK(store.FindFirstAncestorOfClass(leaf, tree.Model) == middle);
	CHECK(store.FindFirstAncestorOfClass(leaf, tree.Part) == NULL_ENTITY);
	CHECK(store.FindFirstAncestorWhichIsA(leaf, tree.PVInstance) == middle);
	CHECK(store.FindFirstAncestorWhichIsA(leaf, ClassId{}) == NULL_ENTITY);
}
...
TEST_CASE("GetFullName is the path from the root down", "[ecs][instance]") {
	...
	CHECK(store.GetFullName(part) == "Workspace.Model.Part");
	CHECK(store.GetFullName(workspace) == "Workspace");

	// A detached instance is its own root, so its path is just its name.
	store.SetParent(model, NULL_ENTITY);
	CHECK(store.GetFullName(part) == "Model.Part");

	CHECK(store.GetFullName(store.Create("plain")).empty());
	CHECK(store.GetFullName(NULL_ENTITY).empty());
}
...
TEST_CASE("renaming an instance changes what finds it", "[ecs][instance]") {
	...
	const Entity parent = store.CreateInstance(tree.Model, "Parent");
	const Entity child = store.CreateInstance(tree.Part, "Before");
	store.SetParent(child, parent);

	REQUIRE(store.SetInstanceName(child, "After"));
	CHECK(store.InstanceNameOf(child) == Name("After"));
	CHECK(store.FindFirstChild(parent, "After") == child);
	CHECK(store.FindFirstChild(parent, "Before") == NULL_ENTITY);
	CHECK(store.GetFullName(child) == "Parent.After");

	// An empty name unnames it rather than naming it the empty string, which is
	// the same reading `CreateInstance` gives one.
	REQUIRE(store.SetInstanceName(child, ""));
	CHECK_FALSE(store.InstanceNameOf(child).IsValid());

	CHECK_FALSE(store.SetInstanceName(store.Create("plain"), "Nope"));
	CHECK_FALSE(store.SetInstanceName(NULL_ENTITY, "Nope"));
}
...
TEST_CASE("Clone skips what is not archivable", "[ecs][instance]") {
	...
	const Entity model = store.CreateInstance(tree.Model, "Model");
	const Entity kept = store.CreateInstance(tree.Part, "Kept");
	const Entity skipped = store.CreateInstance(tree.Part, "Skipped");
	const Entity underSkipped = store.CreateInstance(tree.Part, "UnderSkipped");
	store.SetParent(kept, model);
	store.SetParent(skipped, model);
	store.SetParent(underSkipped, skipped);

	store.Set<engine::ecs::NotArchivable>(skipped, engine::ecs::NotArchivable{});

	const Entity copy = store.CloneInstance(model);
	REQUIRE(copy != NULL_ENTITY);

	CHECK(store.FindFirstChild(copy, "Kept") != NULL_ENTITY);

	// The subtree goes with it: a child of something that was not copied cannot
	// have been copied either.
	CHECK(store.FindFirstChild(copy, "Skipped") == NULL_ENTITY);
	CHECK(store.FindFirstChild(copy, "UnderSkipped", true) == NULL_ENTITY);

	// The source is untouched.
	CHECK(store.FindFirstChild(model, "Skipped") == skipped);

	SECTION("a root that is not archivable clones to nothing") {
		CHECK(store.CloneInstance(skipped) == NULL_ENTITY);
	}
}

TEST_CASE("a clone rewires references that pointed inside it", "[ecs][instance]") {
	// **The line Roblox draws, and the only one that makes both cases work.** A
	// weld naming the two parts it joins has to name the two *new* parts, or
	...
	const Entity model = store.CreateInstance(tree.Model, "Model");
	const Entity left = store.CreateInstance(tree.Part, "Left");
	const Entity right = store.CreateInstance(tree.Part, "Right");
	store.SetParent(left, model);
	store.SetParent(right, model);

	// Inside the subtree, and across it: `left` names its sibling.
	store.Set<Link>(left, Link{right});

	// Out of the subtree, at something the copy must keep sharing.
	store.Set<Link>(right, Link{outside});

	const Entity copy = store.CloneInstance(model);
	REQUIRE(copy != NULL_ENTITY);

	const Entity copiedLeft = store.FindFirstChild(copy, "Left");
	const Entity copiedRight = store.FindFirstChild(copy, "Right");
	REQUIRE(copiedLeft != NULL_ENTITY);
	REQUIRE(copiedRight != NULL_ENTITY);

	// Rewritten: the copy's left names the copy's right, not the original's.
	CHECK(store.Get<Link>(copiedLeft)->Target == copiedRight);
	CHECK(store.Get<Link>(copiedLeft)->Target != right);

	// Left alone: there is one `Outside` and both of them point at it.
	CHECK(store.Get<Link>(copiedRight)->Target == outside);

	// And the source is untouched by any of it.
	CHECK(store.Get<Link>(left)->Target == right);
	CHECK(store.Get<Link>(right)->Target == outside);

	SECTION("a reference to the subtree root itself") {
		store.Set<Link>(left, Link{model});
		const Entity second = store.CloneInstance(model);
		REQUIRE(second != NULL_ENTITY);

		const Entity secondLeft = store.FindFirstChild(second, "Left");
		REQUIRE(secondLeft != NULL_ENTITY);
		CHECK(store.Get<Link>(secondLeft)->Target == second);
	}
}
...
TEST_CASE("FindFirstChild searches one level and reports nothing honestly", "[ecs][instance]") {
	...
	// **The empty name finds nothing, and it used to find the unnamed one.**
	// `CreateInstance("")` leaves an instance with no `core::Name` at all and
	// `InstanceNameOf` answers with an invalid one, which is what `Name("")`
	// compares equal to — so searching for "" was asking for "whatever has no
	// name". Roblox returns nil, and so does this.
	CHECK(store.FindFirstChild(parent, "") == NULL_ENTITY);
	CHECK(unnamed != NULL_ENTITY);
}
