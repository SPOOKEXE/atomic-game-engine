...
// window. `engine/nodegraph/Graph.hpp` carries the arguments; what is here is the
// mechanism.
...
namespace engine::nodegraph {
	...
	bool Graph::Remove(NodeId id) {
		...
		for (engine::nodegraph::Group &frame : Frames) {
			frame.Members.erase(
		...
		Frames.erase(
			std::remove_if(
				Frames.begin(),
				Frames.end(),
				[](const engine::nodegraph::Group &frame) { return frame.Members.empty(); }
			),
			Frames.end()
		);
	...
	bool Graph::Ungroup(GroupId group, bool withNodes) {
		const auto found =
			std::find_if(Frames.begin(), Frames.end(), [&](const engine::nodegraph::Group &frame) {
	...
	engine::nodegraph::Group *Graph::FindGroup(GroupId group) {
		const auto found =
			std::find_if(Frames.begin(), Frames.end(), [&](const engine::nodegraph::Group &frame) {
				return frame.Id == group;
			});
	...
	const engine::nodegraph::Group *Graph::FindGroup(GroupId group) const {
	...
	GroupId Graph::GroupOf(NodeId node) const {
		for (const engine::nodegraph::Group &frame : Frames) {
	...
	std::vector<NodeId> Graph::Absorb(const Graph &other, float dx, float dy, NodeId owner) {
		...
		for (const engine::nodegraph::Group &frame : other.Groups()) {
