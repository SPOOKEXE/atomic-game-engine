...
TEST_CASE("the default PBR document carries material emission and ambient occlusion", "[graph]") {
	...
	CHECK(graph.Find(compiled.PerView[15])->Kind == Name("tonemap"));
	CHECK(graph.Find(compiled.PerView[16])->Kind == Name("portal-overlay"));
	CHECK(graph.Find(compiled.PerView[17])->Kind == Name("mirror-overlay"));
	CHECK(graph.Find(compiled.PerView[18])->Kind == Name("transparent"));
