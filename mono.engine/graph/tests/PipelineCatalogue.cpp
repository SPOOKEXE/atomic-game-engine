...
TEST_CASE("the standard frame's kinds are all registered", "[graph][catalogue]") {
	Kinds();

	// **The six `StandardDocument` names, checked here rather than assumed.** A
	// document naming a kind the catalogue does not hold opens as a box with no
	// ports at all — which is the same symptom as the bug above and a different
	// cause, so it gets its own line.
	for (const char *name : {"shadow", "surface", "opaque", "transparent", "overlay", "interface"}) {
		INFO("kind: " << name);
		CHECK(NodeCatalogue::Find(Name(name)) != nullptr);
	}
}

TEST_CASE("a kind's slot count matches what the standard frame binds", "[graph][catalogue]") {
	Kinds();
	...
	// bindings past the end. `StandardDocument` is the caller that would.
...
TEST_CASE("registering a kind twice replaces it", "[graph][catalogue]") {
	...
	CHECK(NodeCatalogue::Find(Name("opaque"))->Label == "Replaced");
	...
	Kinds();
	CHECK(NodeCatalogue::Find(Name("opaque"))->Label == "Opaque");
