...
TEST_CASE("the standard frame reports no warnings", "[graph][diagnostics]") {
	Kinds();

	const std::vector<Diagnostic> found = Diagnose(engine::graph::StandardGraph());
...
TEST_CASE("a target wider than its readers is reported", "[graph][diagnostics]") {
	...
	// `gbuffer` declares its normal slot as `RGB10A2` — 32 bits — and `present`
	// takes `RGB10A2`. So a frame whose G-buffer feeds a reader wanting fewer
	// bits than it writes is the fault. `opaque` writes `RGBA16F` at 64 bits;
	// `smaa-edges` takes `RGB10A2` at 32.
	const ResourceId colour = Declare(graph, "colour", ResourceKind::Colour, ResourceFormat::RGBA16F);
	...
	CHECK(Names(found, DiagnosticKind::FormatOverspend, "opaque"));
...
TEST_CASE("an alpha channel no reader takes is reported", "[graph][diagnostics]") {
	...
	// arranged to look at `opaque`'s alpha. A first version of this case used
	...
	const std::vector<Diagnostic> found = Diagnose(graph);
	CHECK(Names(found, DiagnosticKind::UnusedAlpha, "opaque"));
