...
TEST_CASE("resource kinds have stable diagnostic names", "[graph]") {
	CHECK(std::string(engine::graph::Describe(ResourceKind::Colour)) == "colour");
	CHECK(std::string(engine::graph::Describe(ResourceKind::Depth)) == "depth");
	CHECK(std::string(engine::graph::Describe(ResourceKind::Texture)) == "texture");
	CHECK(std::string(engine::graph::Describe(ResourceKind::Storage)) == "storage");
	CHECK(std::string(engine::graph::Describe(ResourceKind::Buffer)) == "buffer");
	CHECK(std::string(engine::graph::Describe(ResourceKind::Camera)) == "camera");
...
TEST_CASE("the default frame compiles and its shadow pass is shared", "[graph]") {
	...
	// real frame: world input, shadow and environment work every view samples, twenty passes each view
	// draws for itself, and the window's overlay and chrome once over the lot.
	CHECK(compiled.Shared.size() == 5);
	CHECK(compiled.PerView.size() == 22);
...
TEST_CASE("four views pay for one shadow pass and four colour passes", "[graph]") {
	...
	for (const std::string &line : recorder.Ran) {
		...
		geometry += line.rfind("gbuffer@", 0) == 0 ? 1 : 0;
	}
	CHECK(shadows == 1);
	CHECK(geometry == 4);
	...
	// the frame after it.
	CHECK(recorder.Ran.front() == "world");
}
...
TEST_CASE("one view's passes are adjacent rather than interleaved", "[graph]") {
	...
	const auto first = std::find(recorder.Ran.begin(), recorder.Ran.end(), "camera@0");
	const auto second = std::find(recorder.Ran.begin(), recorder.Ran.end(), "camera@1");
...
TEST_CASE("declaration order is the order, and a read below its write is refused", "[graph]") {
	...
	// out. **A read-modify-write chain is why dependency sorting is not the
	// execution model**: `opaque`,
...
TEST_CASE("an external resource may enter the graph through a read", "[graph]") {
	RenderGraph graph;
	const ResourceId history =
		graph.AddResource({.Name = Name("history"), .Kind = ResourceKind::Texture, .External = true});
	Node composite;
	composite.Name = Name("composite");
	composite.Kind = Name("composite");
	composite.Reads = {history};
	REQUIRE(graph.AddNode(composite).IsValid());
...
TEST_CASE("a runner that fails abandons the frame", "[graph]") {
	...
	// started.
	CHECK(std::find(recorder.Ran.begin(), recorder.Ran.end(), "gbuffer@1") == recorder.Ran.end());
}
...
TEST_CASE("the default frame's scopes say what its nodes are", "[graph]") {
	const RenderGraph graph = DefaultGraph();
...
TEST_CASE("the default frame draws the window's nodes after every view", "[graph]") {
	const RenderGraph graph = DefaultGraph();
	...
	CHECK(std::count(recorder.Ran.begin(), recorder.Ran.end(), "gbuffer@1") == 1);
}
...
TEST_CASE("two views of one world pay for one shadow pass", "[graph]") {
	...
	CHECK(std::count(recorder.Ran.begin(), recorder.Ran.end(), "gbuffer@0") == 1);
	CHECK(std::count(recorder.Ran.begin(), recorder.Ran.end(), "gbuffer@1") == 1);
}
...
TEST_CASE("a world's views run immediately after its shared work", "[graph]") {
	...
	CHECK(at("gbuffer@0") < at("gbuffer@1"));
	CHECK(at("shadow") < at("gbuffer@0"));
}
...
TEST_CASE("views of one world need not be adjacent", "[graph]") {
	...
	CHECK(std::count(recorder.Ran.begin(), recorder.Ran.end(), "gbuffer@0") == 1);
	CHECK(std::count(recorder.Ran.begin(), recorder.Ran.end(), "gbuffer@1") == 1);
	CHECK(std::count(recorder.Ran.begin(), recorder.Ran.end(), "gbuffer@2") == 1);
}
