...
TEST_CASE("the standard frame profiles into a full grid", "[graph][profile]") {
	const PipelineProfile profile = Profiled(engine::graph::StandardGraph());

	REQUIRE(profile.Passes.size() == 6);
	REQUIRE(profile.Resources.size() == 5);
	CHECK(profile.Cells.size() == 30);
	...
	// `opaque` reads what `shadow` wrote.
	CHECK(profile.At(RowOf(profile, "shadow"), ColumnOf(profile, "shadow")) == Access::Write);
	CHECK(profile.At(RowOf(profile, "shadow"), ColumnOf(profile, "opaque")) == Access::Read);
	...
	CHECK(profile.At(RowOf(profile, "colour"), ColumnOf(profile, "transparent")) == Access::ReadWrite);
...
TEST_CASE("a resource's lifetime is where it is written to where it is last read", "[graph][profile]") {
	...
	const auto written = static_cast<uint32_t>(ColumnOf(profile, "shadow"));
	const auto lastRead = static_cast<uint32_t>(ColumnOf(profile, "opaque"));

	CHECK(shadow.FirstWrite == written);
	CHECK(shadow.LastRead == lastRead);
	...
	// memory to something else while `opaque` was still sampling it.
	CHECK(shadow.LiveAt(written));
	CHECK(shadow.LiveAt(lastRead));
	CHECK_FALSE(shadow.LiveAt(lastRead + 1));

	// And it is not alive before anything wrote it.
	if (written > 0) {
		CHECK_FALSE(shadow.LiveAt(written - 1));
	}
}
...
TEST_CASE("the peak is below the sum when lifetimes do not overlap", "[graph][profile]") {
	...
	graph.AddNode(
		{.Name = Name("two"), .Kind = Name("blit"), .Reads = {a}, .Writes = {b}, .Scope = NodeScope::Frame}
	);
	graph.AddNode(
		{.Name = Name("three"), .Kind = Name("blit"), .Reads = {b}, .Writes = {c}, .Scope = NodeScope::Frame}
	);
	graph.AddNode({
		.Name = Name("present"),
		.Kind = Name("present"),
...
TEST_CASE("rows keep declaration order and columns keep execution order", "[graph][profile]") {
	...
	CHECK(profile.Passes[0].Name == Name("shadow"));
	CHECK(RowOf(profile, "colour") > 0);
}
