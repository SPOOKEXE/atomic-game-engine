...
namespace engine::graph {
	...
	//
	// **`StandardGraph` stated as edits**, so the Render Pipeline widget opens
	// on something rather than on nothing, and so the round trip is checked
	// against the real frame rather than only against fixtures.
	//
	// @return The document. `Build`ing it produces a graph that compiles.
