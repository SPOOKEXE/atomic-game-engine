...
namespace launcher {
	...
	void Launcher::InstallControl() {
		...
		BuiltinControlHooks.resize(5);
		std::vector<Entry> manifest;
		manifest.push_back(
			{builtin("architecture"),
			 [this](engine::control::HookRegistration &) { ControlSurface.AddArchitectureTools(); },
			 &BuiltinControlHooks[0]}
		);
		manifest.push_back(
			{builtin("diagnostics"),
			 [this](engine::control::HookRegistration &) { ControlSurface.AddDiagnosticTools(true); },
			 &BuiltinControlHooks[1]}
		);
		manifest.push_back(
			{builtin("resources"),
			 [this](engine::control::HookRegistration &) { ControlSurface.AddStandardResources(); },
			 &BuiltinControlHooks[2]}
		);
		manifest.push_back(
			{builtin("prompts"),
			 [this](engine::control::HookRegistration &) { ControlSurface.AddStandardPrompts(); },
			 &BuiltinControlHooks[3]}
		);
		manifest.push_back(
			{builtin("discovery"),
			 [this](engine::control::HookRegistration &) { ControlSurface.AddDiscoveryTools(); },
			 &BuiltinControlHooks[4]}
		);
